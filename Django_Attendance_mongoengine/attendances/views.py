...
from .models import Attendance, MongoEngineUsers
...
def MongoEngineView(request):
    
    dicts = MongoEngineUsers.objects
    print("dicts: ", dicts)
    # print("MongoEngineUsers.__dict__: ", MongoEngineUsers.__dict__)
    context = {
        "dicts": dicts,
        # "fields": MongoEngineUsers.__dict__
    }
    if request.method == "POST":
        ...
        MongoEngineUser = MongoEngineUsers()
        MongoEngineUser.username = username
        MongoEngineUser.email = email
        
        # MongoEngineUser.last_update =  datetime.datetime.now()

        MongoEngineUser.save()


        return render(request, "attendances/mongoengine.html", {"context":context})
    else:
        return render(request, "attendances/mongoengine.html", {"context":context})