...
def get_all_photos(url):
    ...
    for item_name in soup.findAll('img'):                               # all photos of the user
        photo='http://www.pioneerm.com/tw/'+item_name.get('src')
        print("URL of photo: ", photo)
    # if you want to gather links for a web crawler
    for link in soup.findAll('a'):
        href = link.get('href')
        # title = link.string  # just the text, not the HTML
...
# trade_spider(1)
get_all_photos("http://www.pioneerm.com/tw/")