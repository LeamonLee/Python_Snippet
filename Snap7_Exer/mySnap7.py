...
def DBRead(dev, db_num, length, dbitems):
    ...
    for item in dbitems.values():
        ...
        if _type == 'Real'.lower():
        ...
        elif 'WString'.lower() in _type:
            value = byte_array.get_wstring(data, offset, byte_array.val_len(_type, None))
            ...
            value = byte_array.get_string(data, offset, byte_array.val_len(_type, None))
...
def DBReadTag(dev, db_num, offset, _type):
    ...
    if _type == 'Real'.lower():
        ...
        value = byte_array.get_wstring(data, 0, byte_array.val_len(_type, None))

    elif 'String'.lower() in _type:
        # value = snap7.util.get_string(data, offset, m_dictOffsets["String"])
        # value = snap7.util.get_string(data, 0, byte_array.val_len(_type, None))
        value = byte_array.get_string(data, 0, byte_array.val_len(_type, None))
...
def DBWriteByTag(dev, db_num, item, value):
    ...
    if _type == 'Real'.lower():
        ...
        byte_array.set_wstring(raw_bytearray, 0, value, int((byte_array.val_len(_type, None)/2)-2))

    elif 'String'.lower() in _type:
        # snap7.util.set_string(raw_bytearray, 0, value, byte_array.val_len(_type, None))
        byte_array.set_string(raw_bytearray, 0, value, byte_array.val_len(_type, None)-2)
...
def get_db_size(_dictItems, _dataTypeKey, _offsetKey):
    ...
    nSize = byte_array.val_len(lstTypes[idx], None)
    lastByte = int(str(max(lstOffsets)).split('.')[0]) + nSize
...
def csv2dict(strCsvContent):
    ...
    dictSortedDB_items = {
        item.split(deliminator)[0]: {
            ...
            offsetKey : str(float(item.split(deliminator)[2]))
        } for item in lstFilteredDatas
    }
